...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state):
        ...
        if self.iter_ == self.parallel_env:
            self.iter_ = 0
        self.n_step_buffer[self.iter_].append((state, action, reward, next_state))
        if len(self.n_step_buffer[self.iter_]) == self.n_step:
            state, action, reward, next_state = self.calc_multistep_return(self.n_step_buffer[self.iter_])
    ...
    def sample(self):
        ...
        actions = torch.cat([e.action for e in experiences if e is not None])
        rewards = torch.tensor([e.reward for e in experiences if e is not None]).reshape(-1,1)
        next_states = [e.next_state for e in experiences if e is not None]
#         dones = torch.from_numpy(np.vstack([e.done for e in experiences if e is not None]).astype(np.uint8)).float().to(self.device)

        return (self.state_reco(states), actions, rewards, self.state_reco(next_states))
...
class PrioritizedReplay(object):
    ...
    """
    def __init__(self, args, capacity, batch_size, seed, gamma=0.99, alpha=0.6, beta_start = 0.4, beta_frames=100000, n_step=1, parallel_env=1, ere=False):
        self.args = args
        self.alpha = alpha
...
#         dones       = torch.FloatTensor(dones).to(self.device).unsqueeze(1)
        ...
        return self.state_reco(states), actions, rewards, self.state_reco(next_states), indices, weights
